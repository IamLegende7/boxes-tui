...
class VerticalLayout(Widget):
    ...
    def resize_components(self, new_width:int, new_height:int) -> None: # TODO: implement scroll # TODO: implement diffrent parts of the given space (like -2 = 1/2 of the avalible space) and mixing multiple diffrent ratios
        ...
        if (variable_heights_sum > 1) and (not self.can_scroll):
            log(LogLevel.DEBUG, f'{self.widget_id}: Squishing components...')
            for height_index in range(len(wanted_heights)):
                if wanted_heights[height_index] < 0:
                    old_wanted_height = wanted_heights[height_index]
                    wanted_heights[height_index] = wanted_heights[height_index] * variable_heights_count
                    log(LogLevel.DEBUG, f'   Squishing component {self.components[height_index][0].widget_id} from {old_wanted_height} to {wanted_heights[height_index]}')
        ...
        still_unassinged_height = unassinged_height
        for height_index in range(len(wanted_heights)):
            # width
            ...
            # height
            if wanted_heights[height_index] < 0:
                last_variable_height = True
                for x in wanted_heights[height_index+1:]:
                    if x < 0: last_variable_height = False
                
                if last_variable_height:
                    component_new_height = still_unassinged_height
                else:
                    component_new_height = unassinged_height//-wanted_heights[height_index]
                still_unassinged_height -= component_new_height
            else:
...
class HorizontalLayout(Widget):
    ...
    def extra_init(self, more_args):
        ...
        self.keybind_translator = { # TODO: find a better way
            "menu_function_left":   self.menu_left,
            "menu_function_right":  self.menu_right,
            "menu_function_select": self.menu_select,
            "menu_function_back":   self.menu_back
        }
    ...
    def resize_components(self, new_width:int, new_height:int) -> None: # TODO: implement scroll # TODO: implement diffrent parts of the given space (like -2 = 1/2 of the avalible space) and mixing multiple diffrent ratios
        ...
        # get infos
        wanted_widths = []
        variable_widths_count = 0
        variable_widths_sum = 0
        unassinged_width = self.width
        for component, function in self.components:
            wanted_width = component.wanted_width
            if wanted_width is None:
                wanted_width = -1

            wanted_widths.append(wanted_width)
            if wanted_width < 0:
                variable_widths_count += 1
                variable_widths_sum += 1/-wanted_width
            else:
                unassinged_width -= wanted_width

        # squishing
        if (variable_widths_sum > 1) and (not self.can_scroll):
            log(LogLevel.DEBUG, f'{self.widget_id}: Squishing components...')
            for width_index in range(len(wanted_widths)):
                if wanted_widths[width_index] < 0:
                    old_wanted_width = wanted_widths[width_index]
                    wanted_widths[width_index] = wanted_widths[width_index] * variable_widths_count
                    log(LogLevel.DEBUG, f'   Squishing component {self.components[width_index][0].widget_id} from {old_wanted_width} to {wanted_widths[width_index]}')

        # applying
        log(LogLevel.DEBUG, f'{self.widget_id}: Componentpad is {self.component_pad.getmaxyx()[1]} x {self.component_pad.getmaxyx()[0]}. Size is {self.width} x {self.height}')
        still_unassinged_width = unassinged_width
        for width_index in range(len(wanted_widths)):
            # height
            if (self.components[width_index][0].wanted_height is None) or (self.components[width_index][0].wanted_height == -1):
                component_new_height = floor(self.height)
            else:
                component_new_height = floor(self.components[width_index][0].wanted_height)
            
            # width
            if wanted_widths[width_index] < 0:
                last_variable_width = True
                for x in wanted_widths[width_index+1:]:
                    if x < 0: last_variable_width = False
                
                if last_variable_width:
                    component_new_width = still_unassinged_width
                else:
                    component_new_width = unassinged_width//-wanted_widths[width_index]
                still_unassinged_width -= component_new_width
            else:
                component_new_width = floor(wanted_widths[width_index])

            log(LogLevel.DEBUG, f'Resizing {self.components[width_index][0].widget_id} to {component_new_width} x {component_new_height}')
            self.components[width_index][0].resize(new_width=component_new_width, new_height=component_new_height)
    ...
    def render_components(self, x:int=0, y:int=0) -> None:
        """Renders the Components of the `HorizontalLayout` Widget"""

        full_x = 0
        i = 0
        for component, function in self.components:
            component.render_self(x=full_x, y=0, is_selected=((i == self.selected) and self.show_selected))
            full_x += component.width
            i += 1
        ...
        for component, function in self.components:
            if component.infos.has_components:
                component.render_components(x=full_x, y=y)
            full_x += component.width
            i += 1
    ...
    def update_scroll(self):
        if self.can_scroll and self.count_components > 0:
            prev_x = -self.scroll
            for component, function in self.components[:self.selected]: prev_x += component.width

            if   prev_x+self.components[self.selected][0].width <= 0:
                self.scroll -= self.components[self.selected][0].width
            elif prev_x+self.components[self.selected][0].width > self.width:
                self.scroll += prev_x+self.components[self.selected][0].width - self.width
